...
public:
	Class(const Class & other): ApiDefAutoGen::Namespace(){
		m_root = other.m_root;
	...
	template<class T> Class &addCFunction(const char *name, int(T::*)(lua_State*)) {
		Namespace::addCFunction(name, 0);
		return *this;
	}
...
sTypeName GetTypeOf() {
	int status;
	sTypeName t;
	t.Name = Namespace::TranslateName(abi::__cxa_demangle(typeid(T).name(), 0, 0, &status));
...
inline std::string NameOf() {
	int status;
	return Namespace::TranslateName(abi::__cxa_demangle(typeid(T).name(), 0, 0, &status));
}
...
template<class T>
Namespace& Namespace::addVariable(const char* name, T *, bool Writable) {
...
template <class T, class OWNER>
Class& Class::addData (char const* name, T OWNER::*, bool Writable){
	xml_node node;
